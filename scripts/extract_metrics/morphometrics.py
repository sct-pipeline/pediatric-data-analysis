...
def compute_morphometrics(level_type, output_csv_filename, participants_info, t2w_seg_file, label_file, vertlevels, perlevel=0, perslice=0, pmj=None, normalize_PAM50=0):
    """
    Args : 
    ...
    sct_process_segmentation.main([
        ...
        '-perslice', perslice,
        '-pmj', pmj,
        '-normalize-PAM50', normalize_PAM50,
...
def compute_interpolated_morphometrics(data_path, output_csv_filename, level_type, t2w_pmj_label, t2w_seg_file, participants_info, final_csv_filename, subject):
    """
    This function interpolates PMJ distances at 0.1 intervals between each each Vertebral Level or Spinal Level.
    Then, morphometrics are computed for each distance (1.0, 1.1, 1.2, etc.), the results are saved to a final CSV file.
    ...
    for index, row in csv_file_interpolated.iterrows():
        ...
        temp_csv_filename = os.path.join(data_path, "derivatives/results/morphometrics/",  f"{subject}_temp_pmj_{level_type}_{level}.csv")
    ...
    temp_files_pattern = os.path.join(data_path, "derivatives/results/morphometrics/", f"{subject}_temp_pmj_*.csv")
...
def main(subject, data_path, subject_dir, file_t2):
    ...
    output_csv_filename = os.path.join(data_path, "derivatives/results/morphometrics/",  f"{subject}_morphometrics.csv")
...
if __name__ == "__main__":
    ...
    main(args.subject, args.data_path, args.subject_dir, args.file_t2)
