...
Example usage:
    python scripts/analysis/average_SC_tip_location.py -e /path/to/exclude.yml
""" 

def main(exclude_file):

    # Define the folder containing the `interpolated_morphometrics.csv` files
    morphometrisc_folder = Path("results/tables/morphometrics/")

    # Load exclude.yml file 
    with open(exclude_file, "r") as f:
        exclude = yaml.safe_load(f)

    excluded_subjects = set(exclude.get("sc-tip", []))
    ...
    for csv_file in morphometrisc_folder.glob(f"*{file_suffix}"):
...
if __name__ == "__main__": 
    parser = argparse.ArgumentParser(description="Compute average spinal cord tip location by age.") 
    parser.add_argument("-e", "--exclude-file", type=Path, required=True, help="Path to exclude.yml file") 
    
    args = parser.parse_args() 
    
    main(exclude_file=args.exclude_file)