...
It requires as inputs (in the native T2*w space):
    ...
    - Single-voxel point labels for the inferior limit (most caudal slice) of each spinal level (registered from the spinal levels in the T2w space)

These inputs can be obtained by running `scripts/preprocessing/T2starw_data_preprocessing.sh` and `scripts/preprocessing/T2w_data_preprocessing.sh`. 

This script will output the CSA of GM and WM, at each spinal level along the spinal cord. To do this, it will:
    - Get the slices corresponding to the inferior limit of each spinal level from the single-voxel point labels
    - Compute the slices between each consecutive inferior limit slices to get the slice range corresponding to each spinal level in the T2*w space
...
def compute_GM_WM_distribution(subject, output_dir, spinal_levels_list, spinal_levels_inf_path, sc_seg_path, gm_seg_path, wm_seg_path):
    """
    Compute the GM and WM distribution along the spinal cord for a given subject, using spinal levels (defined by the most caudal slices of the rootlet segmentations)
    """
    ...
    # Load spinal levels inferior limit labels
    ...
    for level in spinal_levels_list:
        ...
        ### Superior limit (which corresponds to the most caudal slice of the previous rootlet)

        # Get the superior z slice where the label equals this level minus 1 level (so it takes the most caudal slice of the previous rootlet)
        sup_level = level - 1 # take the previous level as a starting point for the spinal level
        z_sup = np.where(spinal_levels_inf_img.data == sup_level)[2] - 1 # remove 1 slice (to avoid spinal levels with overlapping slices; the slice already belongs to the previous spinal level)

        # Check if z_sup is empty
        if z_sup.size == 0:
            z_sup = spinal_levels_inf_img.dim[2] - 1
            print(f"Missing superior limit for spinal level {level}. Set z_sup to last slice: {z_sup}")
        else:
            z_sup = int(z_sup[0])
            print(f"z_sup : {z_sup}")

        ### Inferior limit (which corresponds to the most caudal slice of the rootlet)

        # Get the inferior z slice where the label equals this level
        inf_level = level
        z_inf = np.where(spinal_levels_inf_img.data == inf_level)[2]
    ...
    # Compute sct_process_segmentation for all spinal levels combined

    print(f"----- Slices for all spinal levels combined -----")
    
    first_spinal_level = min(spinal_levels_list)
    last_spinal_level = max(spinal_levels_list)

    # Superior limit
    z_sup = np.where(spinal_levels_inf_img.data == (first_spinal_level - 1))[2] - 1 # slice corresponding to the first spinal level (minus 1 to get the previous inferior slice) 
    print(f'z_sup = {z_sup}')
    if z_sup.size == 0:
        z_sup = spinal_levels_inf_img.dim[2] - 1
        print(f"Missing superior limit for spinal level {level}. Set z_sup to last slice: {z_sup}")
    ...
    # Inferior limit    
    z_inf = np.where(spinal_levels_inf_img.data == last_spinal_level)[2] # slice corresponding to the last spinal level
    if z_inf.size == 0:
        z_inf = 0
        print(f"Missing inferior limit for spinal level {level}. Set z_inf to first slice: {z_inf}")
    else:
    ...
    slice_range = f"{z_inf}:{z_sup}"
    print(f'image slice range for all spinal levels : {slice_range}')
...
def main(subject, data_path, file_t2star):
    ...
    # Define the list of spinal levels
    ...
    compute_GM_WM_distribution(subject, output_dir_t2star_seg, spinal_levels_list, spinal_levels_inf_path, sc_seg_path, gm_seg_path, wm_seg_path)

    # Compute GM, WM and SC CSA using the PAM50 atlas registered in the T2*w space
    #compute_GM_WM_distribution(subject, output_dir_PAM50_atlas, spinal_levels_list, spinal_levels_inf_path, sc_atlas_path, gm_atlas_path, wm_atlas_path)
...
if __name__ == "__main__":
    ...
    main(args.subject, args.data_path, args.file_t2star)